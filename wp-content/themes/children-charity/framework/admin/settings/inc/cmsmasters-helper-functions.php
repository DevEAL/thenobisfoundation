...
function children_charity_get_admin_page() {
	global $pagenow;
...
function children_charity_default_tab() {
	$current_page = children_charity_get_admin_page();
...
function children_charity_get_the_tab() {
	global $pagenow;
	
	$default_tab = children_charity_default_tab();
...
function children_charity_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'children-charity') . '</a>';
		}
