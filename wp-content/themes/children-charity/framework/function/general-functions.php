...
function cmsmasters_theme_font($fonts, $font_name = '') {
	...
	if ( 
		...
		if ($local_fonts != '' && $font_name == '') {
			wp_add_inline_style('children-charity-style', $local_fonts);
		}
...
function cmsmasters_get_tweets($username, $count) {
	...
	$numTweets = $count;
	$cacheTime = 1;
	$backupName = 'cmsmasters_' . 'children-charity' . '_tweets_list_backup';
	
	
	$connection = new TwitterOAuth( 
		(($cmsmasters_option['children-charity' . '_api_key'] != '') ? $cmsmasters_option['children-charity' . '_api_key'] : ''), 
		(($cmsmasters_option['children-charity' . '_api_secret'] != '') ? $cmsmasters_option['children-charity' . '_api_secret'] : ''), 
		(($cmsmasters_option['children-charity' . '_access_token'] != '') ? $cmsmasters_option['children-charity' . '_access_token'] : ''), 
		(($cmsmasters_option['children-charity' . '_access_token_secret'] != '') ? $cmsmasters_option['children-charity' . '_access_token_secret'] : '')
	);
...
function children_charity_theme_style_change() {
	if (is_string(get_option('cmsmasters_children_charity_theme_style'))) {
		return;
	} else {
		if (!CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
			update_option('cmsmasters_children_charity_theme_style', '');
		}
