...
/* General Settings */
function children_charity_tribe_events_options_general_fields($options, $tab) {
	$new_options = array();
	
	
	if ($tab == 'content') {
		foreach($options as $option) {
			if ($option['id'] == 'children-charity_search_layout') {		
				$new_options[] = $option;
				
				$new_options[] = array( 
					'section' => 'content_section', 
					'id' => 'children-charity' . '_events_layout', 
					'title' => esc_html__('Events Calendar Layout Type', 'children-charity'), 
					'desc' => '', 
					'type' => 'radio_img', 
					'std' => 'fullwidth', 
					'choices' => array( 
...
add_filter('cmsmasters_options_general_fields_filter', 'children_charity_tribe_events_options_general_fields', 10, 2);
