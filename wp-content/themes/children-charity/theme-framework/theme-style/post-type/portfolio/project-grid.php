...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && children_charity_project_check_exc_cont()) ? true : false;
...
$rollover = in_array('rollover', $cmsmasters_project_metadata) ? false : true;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		children_charity_thumb_rollover(get_the_ID(), $project_thumb_size, false, $rollover, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
					...
					echo '<div class="cmsmasters_project_meta_info entry-meta">';
						...
						if ($categories) {
							echo '<div class="cmsmasters_project_cont_info entry-meta">';
								...
								children_charity_get_project_category(get_the_ID(), 'pj-categs', 'page', 'grid');
				...
				$excerpt ? children_charity_project_exc_cont() : '';
