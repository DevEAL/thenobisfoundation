...
if (
	$cmsmasters_option['children-charity' . '_profile_post_cat'] || 
	$cmsmasters_option['children-charity' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			children_charity_profile_title_nolink(get_the_ID(), 'h1', $cmsmasters_profile_subtitle, 'h5');
		echo '</header>';
	}
		...
		if (get_the_content() != '') {
			echo '<div class="cmsmasters_profile_content entry-content">' . "\n";
				...
				if ($cmsmasters_profile_sharing_box == 'true') {
					children_charity_sharing_box(esc_html__('Like this member?', 'children-charity'), 'h4');
				}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					'<h3 class="profile_details_title">' . esc_html($cmsmasters_profile_details_title) . '</h4>';
					
					children_charity_get_profile_likes('post');
					
					children_charity_get_profile_comments('post');
					
					children_charity_get_profile_features('details', $cmsmasters_profile_features, false, 'h4', true);
					
					children_charity_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			children_charity_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h4', true);
			
			children_charity_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h4', true);
			
			children_charity_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h4', true);
			
			
			children_charity_profile_open_social_icons($cmsmasters_profile_social, $profile_id = false, esc_html__('Socials', 'children-charity'), 'h4');
...
if ($cmsmasters_option['children-charity' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['children-charity' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['children-charity' . '_profile_post_nav_order_cat'] : false);
	
	children_charity_prev_next_posts($order_cat, 'pl-categs');
}
