...
/* Scripts for Admin */
function children_charity_theme_admin_scripts() {
	wp_enqueue_script('children-charity-theme-settings-toggle', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/admin/js/theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('children-charity-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'children-charity' 
	));
}

add_action('admin_enqueue_scripts', 'children_charity_theme_admin_scripts');



/* Register theme JS Scripts */
function children_charity_register_theme_js_scripts() {
	...
	wp_localize_script('children-charity-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'children_charity_register_theme_js_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'children-charity'), 
    'footer' => 	esc_html__('Footer Navigation', 'children-charity'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'children-charity') 
));
...
function children_charity_fallback_menu($args) {
	$cmsmasters_option = children_charity_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['children-charity' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function children_charity_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'children_charity_single_project_layout');



/* Search Form */
function children_charity_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('enter keywords', 'children-charity') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'children_charity_search_form');
