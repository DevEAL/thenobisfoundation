...
function children_charity_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function children_charity_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function children_charity_project_exc_cont($show = true) {
...
function children_charity_project_check_exc_cont() {
	$exc = children_charity_project_exc_cont(false);
...
function children_charity_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $layout_type = 'grid', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['children-charity' . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'children-charity') . ':' . '</div>' . 
...
function children_charity_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['children-charity' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'children-charity') . ':' . '</div>' . 
...
function children_charity_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = children_charity_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = children_charity_get_global_options();
			
			if ($cmsmasters_option['children-charity' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'children-charity') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						children_charity_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function children_charity_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['children-charity' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'children-charity') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function children_charity_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'children-charity') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = children_charity_get_global_options();
		...
		if ($cmsmasters_option['children-charity' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'children-charity') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'children-charity') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function children_charity_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = children_charity_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['children-charity' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'children-charity') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function children_charity_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function children_charity_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = children_charity_get_global_options();
	$out = '';
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'children-charity') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
