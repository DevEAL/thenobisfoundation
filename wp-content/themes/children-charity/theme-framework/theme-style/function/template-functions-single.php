...
function children_charity_prev_next_posts($order_cat = false, $tax_name = 'category') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post">%link<span class="cmsmasters_prev_arrow"><span></span></span><span class="sub">previous</span></span>', '%title', $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post">%link<span class="cmsmasters_next_arrow"><span></span></span><span class="sub">next</span></span>', '%title', $order_cat, '', $tax_name);
...
function children_charity_sharing_box($title_box = false, $tag = 'h3') {
...
function children_charity_author_box($title_box = false, $tag = 'h4', $author_tag = 'h5') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'children-charity') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function children_charity_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
...
function children_charity_author_avatar($template_type = 'page') {
...
function children_charity_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'children-charity') . '</' . esc_html($tag) . '>' . "\n" .
